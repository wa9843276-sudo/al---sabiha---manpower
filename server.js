...
app.get("/api/dashboard",login,(req,res)=>{
 ...
 res.json({total_workers,total_companies:total_companies,total_sites,present_today:present,checked_in:open,todays_attendance:rows,date:d});
});
...
app.post("/api/workers",admin,(req,res)=>{
 try{
 const r=db.prepare(`INSERT INTO workers(worker_code,name,employee_type,vendor,mobile,designation,salary,status,joining_date)
 VALUES(?,?,?,?,?,?,?,'Active',?)`).run(clean(req.body.worker_code),clean(req.body.name),clean(req.body.employee_type)||"Company",clean(req.body.vendor),clean(req.body.mobile),clean(req.body.designation),Number(req.body.salary||0),clean(req.body.joining_date));
...
app.put("/api/workers/:id",admin,(req,res)=>{
 db.prepare(`UPDATE workers SET worker_code=?,name=?,employee_type=?,vendor=?,mobile=?,designation=?,salary=?,status=?,joining_date=? WHERE id=?`)
 .run(clean(req.body.worker_code),clean(req.body.name),clean(req.body.employee_type)||"Company",clean(req.body.vendor),clean(req.body.mobile),clean(req.body.designation),Number(req.body.salary||0),clean(req.body.status)||"Active",clean(req.body.joining_date),Number(req.params.id));
 res.json({success:true});
});
...
app.post("/api/attendance/lookup",login,(req,res)=>{
 ...
 const open=db.prepare(`SELECT a.*,c.name company_name,s.name site_name FROM attendance a LEFT JOIN companies c ON c.id=a.company_id LEFT JOIN sites s ON s.id=a.site_id WHERE a.worker_id=? AND a.attendance_date=? AND a.check_in IS NOT NULL AND a.check_out IS NULL ORDER BY a.id DESC LIMIT 1`).get(w.id,today());
 res.json({worker:w,sites,companies,currently_checked_in:!!open,open_attendance:open||null});
});
app.post("/api/attendance/checkin",login,(req,res)=>{
 ...
 const open=db.prepare("SELECT * FROM attendance WHERE worker_id=? AND attendance_date=? AND check_in IS NOT NULL AND check_out IS NULL").get(w.id,today());
 if(open)return res.status(409).json({error:"Worker is already checked in."});
 const cid=req.body.company_id?Number(req.body.company_id):(site.company_id||null);
...
app.post("/api/attendance/checkout",login,(req,res)=>{
 const w=db.prepare("SELECT * FROM workers WHERE worker_code=? AND status='Active'").get(clean(req.body.worker_code));
 if(!w)return res.status(404).json({error:"Worker not found."});
 const a=db.prepare("SELECT * FROM attendance WHERE worker_id=? AND attendance_date=? AND check_in IS NOT NULL AND check_out IS NULL ORDER BY id DESC LIMIT 1").get(w.id,today());
